...
function(args, resource, output, director) {
  ...
  if (isTRUE(args$fresh) || !identical(resource, director$.cache$last_model)) {
...
#   disable_remember <- function(x)
#     if (is.stagerunner(x)) { x$remember <- FALSE; lapply(x$stages, disable_remember) }
#   disable_remember(stagerunner)
